...
public class PullDoorView extends RelativeLayout {
    ...
    private void setupView() {
        ...
        mScreenWidth = dm.widthPixels; //这里你一定要设置成透明背景,不然会影响你看到底层布局
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                mDelY = mCurryY - mLastDownY; //只准上滑有效
                if (mDelY < 0) scrollTo(0, -mDelY);
                break;
            case MotionEvent.ACTION_UP:
                mCurryY = (int) event.getY();
                mDelY = mCurryY - mLastDownY;
                if (mDelY < 0) if (Math.abs(mDelY) > mScreenHeigh / 3) { //向上滑动超过半个屏幕高的时候 开启向上消失动画
                    startBounceAnim(this.getScrollY(), mScreenHeigh, 450);
                    mCloseFlag = true;
                    callString.getString("");
                } else { //向上滑动未超过半个屏幕高的时候 开启向下弹动动画
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            scrollTo(mScroller.getCurrX(), mScroller.getCurrY()); // 不要忘记更新界面
            postInvalidate();
